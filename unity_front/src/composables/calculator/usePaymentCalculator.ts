...
export const usePaymentCalculator = () => {
  ...
  const calculateAlternative2 = (input: CalculationInput): CalculationResult => {
    ...
    return {
      ...
      paymentSchedule: generateBalloonSchedule(downPayment, monthlyPayment, monthsUntilDeadline, balloonPayment, deadline),
    }
  }
  ...
  const calculateAlternative3 = (input: CalculationInput): CalculationResult => {
    ...
    return {
      ...
      paymentSchedule: generateOneTimeSchedule(downPayment, totalPrice, deadline, input.customPaymentDate, input.customPaymentAmount, input.customPayments),
    }
  }
  ...
  const calculateAlternative4 = (input: CalculationInput): CalculationResult => {
    ...
    return {
      ...
      priceModifier: `-${discountPercent}%`,
      priceModifierValue: -(baseTotal - totalPrice),
      paymentSchedule: generateNegotiatedSchedule(downPayment, remaining, deadline, input.customPaymentDate, input.customPaymentAmount, input.customPayments),
    }
  }
