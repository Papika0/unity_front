<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-light text-slate-900">{{ t('admin.bank_rates.title') }}</h1>
        <p class="text-slate-600 mt-1">{{ t('admin.sidebar.bank_rates') }}</p>
      </div>
      <button
        @click="openAddModal"
    ...
    <div v-else class="bg-white rounded-3xl shadow-xl border border-slate-200 p-12 text-center">
      ...
      <h3 class="text-xl font-medium text-slate-900 mb-2">{{ t('admin.bank_rates.no_banks') }}</h3>
      <p class="text-slate-600 mb-6">{{ t('admin.bank_rates.add_first') }}</p>
      <button
        @click="openAddModal"
