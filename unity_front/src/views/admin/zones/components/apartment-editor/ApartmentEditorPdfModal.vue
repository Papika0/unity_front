    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-in">
      <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-white">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h2 class="text-xl font-semibold text-gray-900">{{ t('admin.zones.apartment_editor.pdf_detection') }}</h2>
              <p class="text-sm text-gray-500 mt-1">{{ t('admin.zones.apartment_editor.pdf_detection_desc') }}</p>
            </div>
          </div>
          <button
            @click="$emit('close')"
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          >
            ...
            <p class="text-sm text-gray-700 mt-2">
              <span class="font-semibold text-red-600">{{ t('admin.common.select') }} PDF</span>
              <span class="text-gray-500"> {{ t('admin.common.drag_drop') }}</span>
            </p>
            <p class="text-xs text-gray-500 mt-1">{{ t('admin.apartments.import.file_types').split(',')[0] }}</p>
          </div>
          <div v-if="pdfFile" class="mt-2 flex items-center justify-between text-sm bg-red-50 p-2 rounded-lg">
            ...
            <button @click="clearPdfFile" class="text-red-600 hover:text-red-700">{{ t('admin.polygon_editor.delete') }}</button>
          </div>
        </div>
        ...
        <div>
          <label class="flex items-center text-sm font-medium text-gray-900 mb-3">
            ...
            {{ t('admin.zones.pdf.clean_image') }}
          </label>
          <p class="text-xs text-gray-500 mb-3">
            {{ t('admin.zones.pdf.clean_image_desc') }}
          </p>
          <div
          ...
          >
            ...
            <p class="text-sm text-gray-700 mt-2">
              <span class="font-semibold text-blue-600">{{ t('admin.common.select_image') }}</span>
              <span class="text-gray-500"> {{ t('admin.common.drag_drop') }}</span>
            </p>
          ...
          <div v-if="targetImageFile" class="mt-2 flex items-center justify-between text-sm bg-blue-50 p-2 rounded-lg">
            ...
            <button @click="clearTargetImageFile" class="text-blue-600 hover:text-blue-700">{{ t('admin.polygon_editor.delete') }}</button>
          </div>
        </div>
        ...
        <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl">
          <div class="flex items-start space-x-2">
            ...
            <div class="text-sm text-amber-800">
              <p class="font-medium">{{ t('admin.zones.pdf.how_it_works') }}</p>
              <ul class="mt-1 list-disc list-inside text-xs space-y-1">
                <li>{{ t('admin.zones.pdf.step1') }}</li>
                <li>{{ t('admin.zones.pdf.step2') }}</li>
                <li>{{ t('admin.zones.pdf.step3') }}</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="p-6 border-t border-gray-200 bg-gray-50 flex items-center justify-end space-x-3">
        ...
        >
          ...
          <span>{{ isPdfDetecting ? t('admin.common.detecting') : t('admin.common.detect') }}</span>
        </button>
      </div>
    </div>
  </div>
